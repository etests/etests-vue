      ...
      <v-col cols="12" class="text-left">
        ...
        >
          <template #item.activationTime="{ item }">
            {{ formatDate(item.activationTime) }}
          </template>
          <template #item.closingTime="{ item }">
            {{ formatDate(item.closingTime) }}
          </template>
          ...
          <template #item.actions="{ item }">
            ...
            <v-btn v-if="item.status <= 1" icon small color="success" disabled="">
              <v-icon small>
                mdi-file-chart
              </v-icon>
            </v-btn>
            ...
            <v-btn rounded outlined small color="indigo" @click="exportTest(item.id)">
              <v-icon small>
                mdi-download
              </v-icon>
              Export
...
export default {
  ...
  watch: {
    declareDialog(newValue, oldValue) {
      if (!newValue) this.$store.cache.dispatch("tests/clearStatus")
    },

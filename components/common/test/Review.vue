<template>
  ...
  >
    <template slot="controls">
      <v-btn text icon small color="primary" @click="$emit('close')">
      ...
      <v-chip rounded class="white primary--text mb-1">
        ...
        <strong> &nbsp; {{ getMinutes(currentAnswer.timeElapsed) }} </strong>
      </v-chip>
    </template>
    ...
    <template slot="text-image">
      ...
      <template v-for="(question, i) in questions">
        <div v-if="questionIndex === i" :key="i" :class="$style.question" v-html="question.text" />
      </template>
    </template>

        ...
        <v-card tile>
          ...
          <v-card-text>
            <v-container>
              ...
              <v-text-field v-model="editedItem.link" solo-inverted flat label="Question Link" />
...
export default {
  ...
  computed: {
    ...
    formTitle() {
      return this.editIndex === -1
        ? "New Question item  "
        : " Edit " + this.questions[this.editIndex].title
    },
    questions: {
      get() {
        if (this.institute) return this.institute.questions
        else return []
      },
      set(questions) {
  ...
  methods: {
    ...
    editItem(item) {
      this.editIndex = this.questions.indexOf(item)
    ...
    save() {
      ...
      if (this.editIndex > -1)
        this.questions = [
          ...this.questions.slice(0, this.editIndex),
          this.editedItem,
          ...this.questions.slice(this.editIndex + 1),
        ]
      else this.questions = this.questions.concat(this.editedItem)
      this.close()
    },
    deleteItem(item) {
      const index = this.questions.indexOf(item)
      if (index !== -1 && confirm("Are you sure you want to delete this item?"))
        this.questions = [...this.questions.slice(0, index), ...this.questions.slice(index + 1)]
    },
